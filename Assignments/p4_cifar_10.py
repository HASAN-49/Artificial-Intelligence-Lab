'''
Problem Statement: Build a simple feedforward neural network to classify the CIFAR-10 dataset.
The CIFAR-10 dataset consists of 32x32 color images of 10 different classes (e.g., airplane, car, bird, cat, deer, dog, frog, horse, ship, truck).
...
def prediction_grid(model, test_X, test_Y):
    ...
    predictions = model.predict(test_X[:16])  # Predict first 16 test samples
    for i in range(16):
        plt.subplot(4, 4, i + 1)
        plt.imshow(test_X[i])
